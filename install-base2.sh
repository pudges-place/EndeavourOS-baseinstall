...
then
  ...
  printf "$message\n" >> logfile2
  printf "Logs are stored in: logfile2\n"
  exit 1
fi
sleep 1
...
# cleanup in case user re-runs script
rmthisdir=$PWD/logfile2
if [ -a $rmthisdir ]
then
   rm -f $rmthisdir
fi
...
cd /etc/netctl
netctl start ethernet-static 2> logfile2
netctl enable ethernet-static 2> logfile2
cd /root
...
printf "\n127.0.0.1\tlocalhost\n" > /etc/hosts
printf "::1\t\tlocalhost\n" >> /etc/hosts
printf "127.0.1.1\t$hos_tname.localdomain\t$host_name\n\n" >> /etc/hosts
...
then
   ...
   grub-install --target=i386-pc $devicename --recheck 2> logfile2
fi
...
sleep 1
curl https://github.com/endeavouros-team/repo/tree/master/endeavouros/x86_64 |grep endeavouros-keyring |sed s'/^.*endeavouros-keyring/endeavouros-keyring/'g | sed s'/pkg.tar.zst.*/pkg.tar.zst/'g |tail -1 > keys
...
then
curl https://github.com/endeavouros-team/repo/tree/master/endeavouros/x86_64 |grep endeavouros-keyring |sed s'/^.*endeavouros-keyring/endeavouros-keyring/'g | sed s'/pkg.tar.xz.*/pkg.tar.xz/'g |tail -1 > keys
file="keys"
read -d $'\04' currentkeyring < "$file"
fi

printf "\nDownloading endeavouros-keyring...\n\n"
message="Downloading endeavouros-keyring... "
wget https://github.com/endeavouros-team/repo/raw/master/endeavouros/x86_64/$currentkeyring 2> logfile2
ok-nok		# function call

printf "\nInstalling endeavouros-keyring... \n\n"
message="pacman - Installing endeavouros-keyring... "
pacman -U --noconfirm $currentkeyring 2> logfile2
ok-nok		# function call
# cleanup
if [ -a $currentkeyring ]
then
...
message="Downloading endeavouros-mirrorlist... "
wget https://github.com/endeavouros-team/repo/raw/master/endeavouros/x86_64/$currentmirrorlist 2> logfile2
ok-nok      # function call

printf "\n\nInstalling endeavouros-mirrorlist... \n\n"
message="pacman - Installing endeavouros-mirrorlist... "
pacman -U --noconfirm $currentmirrorlist 2> logfile2
printf "\nInstalling endeavouros-keyring...   "
ok-nok		# function call
# cleanup
...
message="pacman - Installing yay from EndeavourOS    \n"
...
systemctl enable ufw.service 2>/dev/null #will throw error code because in chroot env, send error to dev null
...
printf "with pacman on your new system, enter\n"
