...
function uefi-auto-partition {
 parted --script -a optimal /$osdevicename \
   mklabel gpt \
   unit mib \
...
readonly scriptversion="Install EndeavourOS base - Version 2020.03.21-x86_64"
uefibootstatus=20
osdevicename="a"
datadevicename="a"
isodeviceno=1
...
then
  ...
  printf "\n${CYAN}Partitioning, formatting, & mounting DATA storage device...${NC}\n"
  ...
  umount $datadevicename &>> logfile1
   printf "\n${CYAN}Partitioning DATA device $datadevicename...${NC}\n"
   message="\nPartitioning DATA devive $datadevicename  " 
   parted --script -a optimal /$datadevicename \
   mklabel msdos \
   unit mib \
   mkpart primary 2MiB $datadevicesize \
   quit
   ok-nok  # function call
...
############################################################
##### the OS device, DATA device, and ENDEAVOUROS ISO device should be mounted at this point
##### any other device attached to the computer should be unmounted to not screw up genfstab
############################################################
devno=1
while [ $devno -le $recordno ]
do
   ...
   devname=$( cat disks | awk -v a="$st" -v b="$sp" 'NR==a,NR==b' | grep "/dev/" | awk '{print $2}')
   devname=("${devname::-1}")
   if [ $devname != $osdevicename ] && [ $devname != $datadevicename ] && [ $devname != $isodevicename ] 
   then
      umount $devname &>> logfile1
   fi
   ((devno=devno+1)) 
done
